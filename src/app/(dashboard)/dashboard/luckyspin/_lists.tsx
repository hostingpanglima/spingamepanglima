...
import { SpinerOptionColumn } from "./_config/column";
interface SpinnerOptionListsProps {
  headerOption: THeaderOption
  data: {
    state: {
      searchFilter: string
      spinerLists: TSpinerOption[]
      currentPage: number
      totalData: number
      totalPage: number
    }
...
const SpinnerOptionLists = ({headerOption,data,selection,EventHandler}:SpinnerOptionListsProps) => {
    ...
    <>
      <Card>
        ...
        <Card className="md:border-none md:bg-transparent md:shadow-none p-3">
          ...
          <div>
            <TableData data={data.state.spinerLists || []} columns={SpinerOptionColumn} selectable sorting selection={selection} />
          </div>
...
export default SpinnerOptionLists;